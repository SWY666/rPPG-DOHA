...
class Dataset_VIPL_HR_Offline(Dataset):
    def __init__(self, frame_path, person_name, version_type, mask_path, wave_path, frame_drop=12, length=70, is_train=True, leave_1=False):
        super().__init__()
        self.length = length
    ...
    def __getitem__(self, index):
        ...
        if self.leave_1:
            _wave_drop = wave_gt_return[self.frame_drop: -self.frame_drop + 1]
        else:
        ...
        return residual_list_return, frame_list_return, map_gt_return, mask_list_return, wave_gt_return, wave_gt_return,\
               self.frame_lists[idx], start, end, name
